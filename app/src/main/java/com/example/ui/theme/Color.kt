package com.example.ui.theme

import androidx.compose.ui.graphics.Color

val BackgroundDark = Color(0xFF121212)
val SurfaceDark = Color(0xFF181818)
val SurfaceCard = Color(0xFF1E1E1E)
val GradientStart = Color(0xFF1E293B)
val GradientEnd = Color(0xFF0F172A)
val TextPrimary = Color(0xFFF8FAFC)
val TextSecondary = Color(0xFF94A3B8) // Slate 400
val AccentAmber = Color(0xFFFBBF24) // Amber 400
val AccentGold = Color(0xFFFFD700) // Gold
val SurfaceElevated = Color(0xFF242424) // Elevated Dark Surface
val BorderSubtle = Color(0x26FFFFFF) // White/15
val BorderGold = Color(0x4DFFD700) // Gold/30
val IconContainer = Color(0xFF2A2A2A)
val AccentCrimson = Color(0xFFEF4444) // Red 500
val AccentEmerald = Color(0xFF10B981) // Emerald 500
val CelestialBlue = Color(0xFF38BDF8) // Sky 400
val ZodiacPurple = Color(0xFF818CF8) // Indigo 400
val StarlightWhite = Color(0xFFFFFFFF)

val DeepNavy = Color(0xFF0A0E1A)
val CosmicBackground = Color(0xFF0A0E1A)
val SurfaceNavy = Color(0xFF131B2E)
val SurfaceNavyCard = Color(0xFF1A233A)
